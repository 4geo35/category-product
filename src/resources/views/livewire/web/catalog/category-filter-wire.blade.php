<form id="formFilter{{ $isModal ? 'Modal' : '' }}" class="space-y-indent mb-indent" wire:submit.prevent="applyFilters">
    @foreach($filters as $filter)
        <div wire:key="{{ $filter->id }}" class="border-b border-stroke pb-indent">
            @switch($filter->type)
                @case("checkbox")
